%% A SVM which gives Posterior Probabilitie is used
%% %%10 Labeled%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% It can be seen that, the SSL does not function well if we use a small
% number of labeled data. This happens, because if we choose a small number
% of data there is a high chance that we are not providing a good sample of
% data. however if we use more of traning data, the algorithm showed to
% perform better than a normal classifier withouth Semi-sepervised Learning
% (at least for this prooblem)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% as it was observed, for some cases the Semi-Superwised Learning performs
% better and for some cases it does not perform well. My conclusion is that, its performance is dependent to the data
% distribution. The SSL is very sampled base. Howver, in case that we have a lack of labeled data, this
% method might be a good option. A good percentage of labeled data needs be
% chosen for the initial data. For the same problem, I used %25 of labeled
% data instead of %15 and the results improved. Overall, it is hard to make
% a conclusion out of this topic.
%sinceley,
